...
1. **Prioritize - First Priority (Failed PR Repair)**: You MUST first query \`list_pull_requests\` across all targeted repositories. Identify any **open Pull Requests created by you** (the agent) that have **failed CI/testing checks** (where \`ciStatus === "failure"\`).
    ...
    - **Prepare Environment File (.env)**: Before running any installation or validation commands, check if a \`.env.example\` file exists in the repository. If it does, dynamically create a \`.env\` file by running a simple copy command (like \`cp .env.example .env\` or \`copy .env.example .env\` on Windows). **DO NOT use \`sed\`, \`awk\`, or shell redirects (like \`>\` or \`>>\`) to write or edit \`.env\`, as they are strictly blocked by security safeguards.** Instead, once copied, inspect the \`.env\` file and use the standard \`replace_lines\` tool to fill any empty, blank, or placeholder environment variables with safe dummy values (e.g., \`API_KEY=dummy_api_key\`) so that CI/CD validation does not fail.
