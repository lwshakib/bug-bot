...
1. **Prioritize**: Use \`list_issues\` and \`list_pull_requests\`.
    - **Duplicate Detection**: Check the state of existing pull requests for the same issue or bug. If there is an **open** PR already created by you:
        - **CI Status check**: Check the \`ciStatus\` of the existing PR. If the \`ciStatus\` is \`failure\`, you MUST NOT skip it. Instead, checkout that PR's branch (\`head\` branch name), inspect the \`failedChecks\` to understand the errors, modify the code to fix the failure, run all validation checks until they pass, and commit/push to update the existing PR branch!
        - If the \`ciStatus\` is \`success\` or \`pending\`, you MUST NOT create a new pull request; use \`send_email\` to report the duplicate and then SKIP processing it.
        - If the existing open PR was created by another contributor, always skip it to avoid duplication.
    - **Closed PRs**: If the existing PR is **closed**, you MUST proceed to fix the issue and create a new pull request for it.
2. **Execute**: For each issue:
    ...
    - **Prepare pnpm Workspace Settings**: If the repository uses \`pnpm\` (detected by \`pnpm-lock.yaml\`), inspect its \`pnpm-workspace.yaml\` file (or create it if it does not exist). Ensure that it contains a \`packages\` block (e.g., listing \`- "."\`) and an \`allowBuilds\` block that explicitly permits/trusts build scripts for key dependencies (e.g., \`@google/genai\`, \`@prisma/engines\`, \`prisma\`, \`protobufjs\`, \`sharp\`, \`unrs-resolver\`). This prevents installation and native build script blockages under GitHub Actions CI.
...
- **Autonomous Setup Requirements**:
    ...
    2. **pnpm Workspace Security**: If using \`pnpm\` (detected by \`pnpm-lock.yaml\`), inspect or create \`pnpm-workspace.yaml\`. Ensure it has \`packages\` configured (e.g. \`- "."\`) and an \`allowBuilds\` section that explicitly trusts scripts for critical libraries (including but not limited to: \`@google/genai\`, \`@prisma/engines\`, \`prisma\`, \`protobufjs\`, \`sharp\`, \`unrs-resolver\`). This prevents dependency/native build failures under GitHub CI.
