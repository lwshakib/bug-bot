...
1. **Prioritize - First Priority (Failed PR Repair)**: You MUST first query \`list_pull_requests\` across all targeted repositories. Identify any **open Pull Requests created by you** (the agent) that have **failed CI/testing checks** (where \`ciStatus === "failure"\`).
    ...
    - **Prepare Environment File (.env)**: Before running any installation or validation commands, check for all \`.env.example\` files in the repository (including both the root and any package or application subdirectories in monorepos). For every \`.env.example\` found, dynamically create a corresponding \`.env\` file in the same directory by running a simple copy command (like \`cp path/to/.env.example path/to/.env\` or \`copy\` on Windows). **DO NOT use \`sed\`, \`awk\`, or shell redirects (like \`>\` or \`>>\`) to write or edit \`.env\` files, as they are strictly blocked by security safeguards.** Once copied, inspect the \`.env\` file(s) and use the standard \`replace_lines\` tool to fill any empty, blank, or placeholder environment variables with safe dummy values (e.g., \`API_KEY=dummy_api_key\`) so that CI/CD validation does not fail.
...
- **Autonomous Setup Requirements**:
    1. **Dummy Env File**: Before executing installation or build/validation commands, check for \`.env.example\`. If it exists, copy it to \`.env\` using a simple \`cp\` (or \`copy\`) command. **Never use \`sed\`, \`awk\`, or redirects (like \`>\` or \`>>\`) in \`run_command\` to create or edit files, as they are blocked. Use the standard \`replace_lines\` tool to modify and fill any empty environment variables.**
...
- **Mismatched Braces & Syntax Precision**: When performing replacements using \`replace_lines\`, be extremely careful to maintain opening and closing braces (\`{\` and \`}\`). Mismatched braces will trigger a SYNTAX GATE failure and block the write. Double check that every block you modify has matching syntax and correct enclosing characters.
</surgical_fix_rules>
`.trim();
